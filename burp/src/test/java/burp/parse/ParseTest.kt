...
class ParseTest {
    ...
    fun testExtractStatementsFromShaclViolation() {
        ...
        assertEquals("http://example.org/Alice", statements[0].stmt.subject.value)
        assertEquals("http://example.org/age", statements[0].stmt.predicate.value)
        assertEquals("twenty", (statements[0].stmt.`object` as Literal).value)
    }
